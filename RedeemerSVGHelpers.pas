...
begin
  ...
  try
    Result := c.GetNextCoordinate(Value); // 02 Jan 2024: erstes Argument -1 entfernt
    //if Result then
    //if Value < 0 then // keine Prozentwerte erlaubt // 02 Jan 2024: entfernt, da es use mit negativem x und y stört
    //Result := False;
  finally
    c.Free;
  end;
